...
- **Anthropic (Claude Agent SDK)Anthropic （Claude Agent SDK）**：通过一个简单的 query() 函数暴露 Harness，运行时是一个“笨循环”，智慧全在模型里。
- **OpenAI (Agents SDK) OpenAI （Agents SDK）**：采用“代码优先”策略，工作流逻辑直接用 Python 表达，而不是复杂的图形语言。
- **LangGraph**：将 Harness 建模为显式的状态图，强调对流程的精细控制。
- **CrewAI**：实现了基于角色的多智能体协作，由“流程层”管理确定性的骨干逻辑。
- **AutoGen 一类气焊**：由微软开发，支持多种编排模式，如顺序执行、群聊、移交和动态任务管理。
