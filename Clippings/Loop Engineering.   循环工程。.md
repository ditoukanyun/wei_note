...
1. **Automations 自动化** that go off on a schedule and do discovery and triage by themselves.
2. **Worktrees** so two agents working in parallel dont step on each other.
3. **Skills 技能** to write down the project knowledge the agent would otherwise just guess.
