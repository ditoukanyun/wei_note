...
- 页面结构是否优先使用语义化标签，而不是全部使用 `div`。
- 表单控件是否配套 `label`，提升可访问性和点击范围。
- 图片是否有合适的 `alt`，装饰性图片是否避免干扰读屏。
- 链接、按钮、输入框的语义是否符合实际行为。
- iframe 是否限制来源、权限和尺寸，避免安全与体验问题。

## 案例

文章详情页可以使用 `header` 放标题和作者信息，`main` 包裹正文，`article` 表示文章主体，`aside` 放相关推荐，`footer` 放版权和导航。语义化结构能帮助搜索引擎、读屏软件和维护者理解页面。
