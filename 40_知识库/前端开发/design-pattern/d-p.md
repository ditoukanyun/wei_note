...
如果只有两个分支，且短期不会变化，直接 `if` 往往更清楚。若支付、校验、折扣、排序等规则会持续增加，并且每种规则都需要独立测试和独立发布，策略模式就能把“选择规则”和“执行规则”拆开。设计模式的目标不是显得高级，而是让变化点更容易替换、组合和验证。

## 检查清单

- 先问变化点在哪里，再决定是否引入模式。
- 模式要减少调用方复杂度，而不是把复杂度换个文件藏起来。
- 单例、发布订阅、中介者等全局协作模式要特别注意生命周期和清理。
- 代理、装饰器、适配器都属于包装思路，但解决的问题不同。
- 与 [[JS-Depth]] 一起复习时，注意闭包、原型、this 与模式实现之间的关系。

## 常见误区

- 为了使用模式而使用模式，导致简单问题被过度抽象。
- 只记 UML 名称，不理解模式背后的变化方向。
- 把设计原则当成绝对规则；工程里要结合复杂度、团队能力和变化频率权衡。
