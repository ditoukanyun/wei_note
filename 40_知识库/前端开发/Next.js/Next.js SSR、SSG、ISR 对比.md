...
渲染策略应由数据新鲜度、个性化程度、页面数量和源站压力共同决定。用户身份强相关的页面不能简单静态化；几乎不变的内容没必要每次请求 SSR；需要周期更新且访问量大的内容适合 ISR 或缓存再验证。

在 App Router 中还要区分服务端组件、客户端组件、fetch 缓存和路由段配置。一次错误的缓存配置可能让用户看到过期数据，或让本可缓存的页面持续打到源站。
