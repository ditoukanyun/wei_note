...
![mixin](40_知识库/前端开发/vue/img/28.png)
...
![mixin1](40_知识库/前端开发/vue/img/29.png)
...
![mixin2](40_知识库/前端开发/vue/img/30.png)
...
![v-if](40_知识库/前端开发/vue/img/31.png)

- 可以看的，此时判断都为false，v-if用注释节点来占位，而v-show直接设置为`display:none`

![v-show](40_知识库/前端开发/vue/img/32.png)
...
![display](40_知识库/前端开发/vue/img/33.png)

选中display，此时页面根本没有出现其位置，也就意味着不存在页面中，不占位置，当需要显示的时候会触发回流和重绘

![visibility](40_知识库/前端开发/vue/img/34.png)
...
- 是否区分 v-if 和 v-show 的渲染成本与切换成本。
- 组件通信是否优先选择清晰的 Props/Emit，再考虑全局状态。
- 插槽、动态组件和 keep-alive 是否有明确使用场景。
- 表单双向绑定是否理解事件、值和修饰符的关系。
- 函数式组件是否只用于无状态、轻量透传或包装场景。

## 案例

后台管理系统的列表页可以用 Props 接收筛选条件，用 Emit 通知父组件刷新，用插槽暴露自定义列，用 keep-alive 缓存返回列表时的状态。把这些基础能力组合起来，比单独背 API 更接近真实开发。
