...
- 父传子是否优先用 Props，子传父是否优先用 Emit。
- 是否避免 `$parent`、`$children` 造成强耦合。
- Provide/Inject 是否用于稳定上下文，而不是随意共享所有状态。
- 全局状态是否只承载真正跨页面或跨模块的数据。
- 事件总线是否有注销逻辑，避免内存泄漏。

## 案例

表格组件可以用 Props 接收数据和列配置，用 Emit 通知分页、排序和筛选变化；若表格内部有多层单元格组件，可以用 Provide/Inject 传递表格上下文，避免每一层都透传重复参数。
