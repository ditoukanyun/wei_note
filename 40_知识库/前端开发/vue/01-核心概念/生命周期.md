...
![生命周期](40_知识库/前端开发/vue/img/7.png)

## beforeCreate

![beforeCreate](40_知识库/前端开发/vue/img/8.png)
...
![created](40_知识库/前端开发/vue/img/9.png)
...
![beforeMount](40_知识库/前端开发/vue/img/10.png)
...
![mounted](40_知识库/前端开发/vue/img/11.png)
...
![beforeUpdate](40_知识库/前端开发/vue/img/12.png)
...
- 初始化数据和订阅是否放在合适阶段。
- DOM 依赖逻辑是否等到 `mounted` 后执行。
- 更新阶段是否避免再次同步修改同一状态导致循环。
- 销毁阶段是否清理定时器、事件监听和外部订阅。
- keep-alive 场景是否处理 `activated/deactivated`。

## 常见误区

- 在 `created` 中操作 DOM。
- 在 `updated` 中无条件修改响应式数据。
- 忘记在销毁前清理自定义事件或定时器。
