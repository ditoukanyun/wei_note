...
![构造组件过程](40_知识库/前端开发/vue/img/14.png)
...
![解析过程](40_知识库/前端开发/vue/img/15.png)
...
![dom](40_知识库/前端开发/vue/img/16.png)

这样子就把一个简单的虚拟DOM还原成真实DOM了

![构建组件过程](40_知识库/前端开发/vue/img/13.png)
...
- 是否理解 VNode 是真实 DOM 的轻量描述。
- 是否知道 key 会影响列表 diff 和节点复用。
- 是否避免用数组下标作为不稳定列表 key。
- 是否理解虚拟 DOM 不是一定更快，而是让 UI 更新更可预测。
- 是否能解释同层比较和组件边界对 diff 的影响。

## 常见误区

- 认为虚拟 DOM 会自动让所有操作更快。
- 列表 key 不稳定，导致状态错位。
- 把 diff 理解成全树任意节点最优比较，忽略同层比较策略。
