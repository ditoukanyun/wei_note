...
![mvvm](40_知识库/前端开发/vue/img/1.png)
...
![响应式](40_知识库/前端开发/vue/img/2.png)

- 数据响应式的任务就是，监听数据的改变，同步渲染view视图的变化，也就是数据变化DOM也更新内容

### 数据劫持

![数据劫持](40_知识库/前端开发/vue/img/3.png)
...
![依赖收集](40_知识库/前端开发/vue/img/4.png)
...
![数据响应式原理](40_知识库/前端开发/vue/img/5.png)
...
![双向绑定](40_知识库/前端开发/vue/img/6.png)
...
- 是否理解 getter 读取时收集依赖，setter 写入时触发更新。
- 是否区分 Vue 2 的 `Object.defineProperty` 和 Vue 3 的 Proxy。
- 是否知道数组、对象新增属性等响应式边界。
- 是否能解释 Watcher、Dep、Observer 在简化实现中的职责。
- 是否避免在渲染过程中产生难以追踪的副作用。

## 常见误区

- 认为“数据变了视图就立刻同步更新”，忽略调度和批量更新。
- 把响应式对象随意解构，导致丢失响应式连接。
- 在 watcher 中继续修改同一依赖，造成循环更新。

## 掘金文章补充

掘金文章《Vue3 响应式 reactive、ref、effect 流程解析》把 Vue3 响应式拆成三个核心动作：`reactive/ref` 负责把原始数据包装成可追踪对象，`effect` 负责声明依赖数据的副作用函数，`track/trigger` 负责在读取时收集依赖、写入时触发依赖。和 Vue2 的 `Object.defineProperty` 相比，Vue3 更依赖 `Proxy + Reflect` 对对象访问进行统一代理。

理解时不要把响应式等同于模板更新。响应式系统本身可以脱离视图存在：只要某段逻辑在 `effect` 中读取了响应式值，它就会被收集；后续写入值时，系统触发对应 effect。模板渲染只是 effect 的一个典型应用场景。

来源：[Vue3 响应式 reactive、ref、effect 流程解析](https://juejin.cn/post/7043265644594200613)
