...
```mermaid
flowchart LR
  A[页面卡顿或顺序异常] --> B[确认当前宏任务]
  B --> C[检查微任务队列是否被持续填充]
  C --> D[观察网络、定时器、事件回调来源]
  D --> E[确认是否阻塞渲染或输入响应]
  E --> F[拆分任务、让出主线程、减少同步计算]
```

## 案例：Promise 递归导致页面无法渲染

如果某段代码在 `Promise.then` 内不断创建新的 `Promise.then`，微任务队列会在当前宏任务结束前持续被清空又填满。浏览器没有机会进入下一轮事件循环，也就无法执行渲染、输入响应和定时器回调。排查时可以先暂停脚本，看调用栈是否反复停留在同一个微任务链路，再用分片任务、`setTimeout`、`requestAnimationFrame` 或 Web Worker 把长任务拆开。

## 检查清单

- 异步顺序异常时，先判断回调来自宏任务还是微任务。
- 页面卡顿时，检查是否存在长同步任务或无限微任务链。
- 网络请求回调不要直接触发大规模 DOM 修改，应先合并状态再批量渲染。
- 定时器不能保证精确时间，只能保证回调进入任务队列的最早时间。
- 和 [[Browser]]、[[BrowersPlus1]] 一起复习时，重点串起进程、线程、事件循环与渲染管线。

## 常见误区

- 误以为 `setTimeout(fn, 0)` 会立即执行；它仍然需要等待当前宏任务和微任务完成。
- 误以为微任务越快越好；过多微任务会饿死渲染和用户输入。
- 误把 XHR、定时器、DOM 事件都理解为同一种异步来源；它们进入任务队列的线程和触发条件不同。
