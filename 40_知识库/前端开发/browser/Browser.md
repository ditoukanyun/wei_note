...
![进程间通信](40_知识库/前端开发/browser/img/1.png)
...
![webkit架构](40_知识库/前端开发/browser/img/2.png)
...
![浏览器内核](40_知识库/前端开发/browser/img/3.png)
...
![GUI渲染](40_知识库/前端开发/browser/img/4.png)
...
![解析html](40_知识库/前端开发/browser/img/5.png)
...
![stylesheet](40_知识库/前端开发/browser/img/8.png)
...
![重排](40_知识库/前端开发/browser/img/12.png)
...
![重绘](40_知识库/前端开发/browser/img/13.png)
...
![事件循环](40_知识库/前端开发/browser/img/14.png)
...
![事件循环1](40_知识库/前端开发/browser/img/15.png)
...
![事件判断](40_知识库/前端开发/browser/img/20.png)
...
![js引擎](40_知识库/前端开发/browser/img/16.png)

以下是异步任务队列大致结构

![任务队列](40_知识库/前端开发/browser/img/17.png)

以下是WebApis大致结构

![webapis](40_知识库/前端开发/browser/img/18.png)

以上是事件循环中需要涉及到的部分，接下来看看事件循环机制(**请放大查看**)

![js事件循环机制](40_知识库/前端开发/browser/img/19.jpg)
...
```mermaid
flowchart TD
  A[输入 URL] --> B[浏览器进程解析地址]
  B --> C[网络进程发起 DNS/TCP/TLS/HTTP]
  C --> D[渲染进程解析 HTML]
  D --> E[构建 DOM/CSSOM]
  E --> F[布局、绘制、合成]
  F --> G[执行 JS 与事件循环]
...
遇到首屏慢，不要只盯着 JS 包体积。先在 Network 面板看 DNS、TLS、TTFB、下载时间，再在 Performance 面板看 HTML 解析、样式计算、布局和脚本执行。若 TTFB 高，优先查后端和 CDN；若下载慢，查缓存、压缩和资源拆分；若主线程长任务多，再定位 JS 执行、布局抖动和同步计算。

## 检查清单

- 先区分问题发生在网络阶段、解析阶段、渲染阶段还是交互阶段。
- 关注主线程长任务，超过 50ms 的任务会影响输入响应。
- 静态资源要检查缓存命中、压缩、优先级和阻塞关系。
- JS 修改样式后立即读取布局信息，可能触发强制同步布局。
- 与 [[net]]、[[BrowersPlus1]]、[[BrowersPlus2]] 联动复习，可以把浏览器链路从协议层串到渲染层。

## 常见误区

- 误把浏览器理解成单线程；单线程主要指渲染进程里的 JS 主线程。
- 误以为 DOM 构建完成就等于页面可用；CSS、字体、脚本和合成也会影响可见与可交互。
- 误以为所有性能问题都靠框架优化解决；很多瓶颈来自网络、缓存、布局和资源优先级。
