...
![查找IP](40_知识库/前端开发/browser/img/21.png)
...
![浏览器准备](40_知识库/前端开发/browser/img/22.png)
...
![socket](40_知识库/前端开发/browser/img/23.png)
...
![网络数据传递过程](40_知识库/前端开发/browser/img/24.png)
...
![互联网](40_知识库/前端开发/browser/img/25.png)
...
![绘制列表](40_知识库/前端开发/browser/img/26.png)
...
- 是否区分浏览器进程、网络进程、渲染进程和 GPU 进程。
- 性能排查时是否对应到 DNS、下载、解析、布局、绘制和合成阶段。
- 是否避免频繁触发布局和重绘。
- 图片、CSS、JS 加载顺序是否影响首屏。
- 是否用 DevTools Performance 验证渲染瓶颈。

## 案例

页面滚动卡顿时，可在 Performance 面板查看是否大量 Layout 和 Paint。如果是 JS 频繁修改布局属性，应减少同步读写布局或使用 transform 优化。

## 常见误区

- 把所有页面慢都归因于 JS 执行。
- 忽略 CSS 和图片资源对渲染关键路径的影响。
- 过度使用合成层，导致内存和 GPU 压力上升。
