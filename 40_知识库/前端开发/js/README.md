...
JavaScript 在前端项目中承担状态计算、事件响应、网络请求、组件逻辑和运行时错误处理。掌握 JS 不能只停留在语法，还要理解闭包、this、原型链、Promise、模块加载和浏览器运行机制，这些知识直接影响 [[React 基础]]、[[MOC-Vue]] 和 [[前端性能优化清单]] 的实现质量。

## 检查清单

- 是否能解释执行上下文、作用域链和闭包。
- 是否能区分同步任务、宏任务、微任务和渲染时机。
- 是否理解原型链、类语法和对象继承的关系。
- 是否能写出可维护的异步流程和错误处理。
- 是否知道何时引入 [[TypeScript 工程实践总览]] 来约束复杂业务模型。
