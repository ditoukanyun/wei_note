...
![ ToPrimitive转换](40_知识库/前端开发/js/img/4.png)
...
看看函数的原型链
![函数原型链](40_知识库/前端开发/js/img/18.png)
...
![原型继承](40_知识库/前端开发/js/img/24.png)
...
先看一张图
![函数this](40_知识库/前端开发/js/img/17.png)
...
执行前
![this演示1](40_知识库/前端开发/js/img/15.png)

执行后
![this演示2](40_知识库/前端开发/js/img/16.png)
...
先看一张图，有个大体架构
![事件循环](40_知识库/前端开发/js/img/22.png)
...
![事件循环](40_知识库/前端开发/js/img/23.png)

以下为主线程判断逻辑

![事件判断](40_知识库/前端开发/js/img/21.png)
...
![location属性](40_知识库/前端开发/js/img/6.png)
...
location方法
![location方法](40_知识库/前端开发/js/img/5.png)
...
![cookie](40_知识库/前端开发/js/img/2.png)
...
![session](40_知识库/前端开发/js/img/8.png)
...
![storage](40_知识库/前端开发/js/img/14.png)
...
响应内容
![jsonp](40_知识库/前端开发/js/img/3.png)
...
![setTimeout](40_知识库/前端开发/js/img/12.png)

- 以上使用嵌套setTimeout来实现循环调用，可以从中看出setTimeout计时是从上一个setTimeout回调执行之后开始的，看看代码效果

![setTimeout代码演示](40_知识库/前端开发/js/img/13.png)
...
![setInterval](40_知识库/前端开发/js/img/9.png)

- 从上图可以先得出结论，setInterval的延迟执行时间包含自身回调执行所占用的时间，看看代码效果

![setInterval代码演示](40_知识库/前端开发/js/img/10.png)

- 上图计算的是2次调用回调之间的间隔，不包括回调执行时间，可以看出setInterval在2次执行之间的延迟受到了回调的影响，再验证一下

![setInterval代码演示1](40_知识库/前端开发/js/img/11.png)
...
![requestAnimationFrame](40_知识库/前端开发/js/img/7.png)
...
![事件传递](40_知识库/前端开发/js/img/20.png)
...
## 实践流程：排查 JavaScript 执行问题

```mermaid
flowchart LR
  A[出现异常或行为不符合预期] --> B[确认作用域与 this]
  B --> C[确认同步/异步执行顺序]
  C --> D[检查原型链与对象引用]
  D --> E[检查 DOM 事件传播]
  E --> F[补最小复现和边界用例]
...
事件委托中如果直接使用 `event.target` 删除节点，点击 `li` 内部的 `span` 时，删除的可能是 `span` 而不是目标 `li`。更稳妥的方式是用 `closest('li')` 找到符合条件的目标节点，并确认它仍属于当前委托容器。这个案例把事件传播、目标元素、当前处理元素和 DOM 查询边界都串在一起。

## 检查清单

- `this` 问题先确认调用位置，而不是函数定义位置。
- 异步问题先画出同步代码、微任务、宏任务和渲染时机。
- 原型问题先区分自有属性、原型属性和属性遮蔽。
- DOM 事件问题先确认捕获、目标、冒泡三个阶段。
- 内存问题重点查闭包持有、定时器、事件监听和缓存引用。
- 与 [[Browser]]、[[BrowersPlus2]] 联动复习时，把 JS 执行模型放回浏览器事件循环里理解。

## 常见误区

- 认为箭头函数只是写法更短，忽略它没有自己的 `this`。
- 认为 `addEventListener` 的匿名函数可以随时移除；移除需要同一个函数引用。
- 只背执行顺序题，不把它用于真实的 UI 卡顿、竞态和状态更新排查。
