...
## 实践流程：编写可维护的 JavaScript

```mermaid
flowchart LR
  A[明确输入输出] --> B[拆分纯逻辑与副作用]
  B --> C[命名表达业务意图]
  C --> D[处理异常和边界]
  D --> E[补充测试或示例]
  E --> F[性能热点再做专项优化]
...
## 案例：WorkerPool 适合什么场景

WorkerPool 适合 CPU 密集型任务，例如大数组统计、图片处理、加密计算、语法分析等。它不适合普通网络请求或少量 DOM 操作，因为 Worker 与主线程之间存在消息传递成本，DOM 也不能直接在 Worker 中访问。使用前应先确认主线程确实被计算阻塞，再评估数据复制、`SharedArrayBuffer`、任务粒度和错误回收策略。

## 检查清单

- 函数先写清楚输入、输出、错误条件，再考虑抽象。
- 复杂异步流程要区分任务调度、状态更新和结果合并。
- 大计算优先评估是否能分片、缓存、懒执行或移到 Worker。
- 对共享状态保持谨慎，必要时用不可变数据或明确的状态机。
- 与 [[JS-Depth]] 联动复习时，把语法、执行模型、内存和浏览器线程一起看。

## 常见误区

- 过早优化：没有性能证据就引入 Worker、缓存或复杂调度。
- 把异步写成嵌套回调，导致错误处理和取消逻辑失控。
- 只追求代码短，忽略命名、边界和维护者阅读成本。
