...
- 是否区分会修改原数组的方法和返回新数组的方法。
- `map`、`filter`、`reduce` 是否表达清晰，避免过度链式调用。
- `sort` 是否提供正确比较函数。
- `reduce` 是否设置初始值，避免空数组报错。
- 大数组处理是否评估时间复杂度和内存占用。

## 案例

订单列表筛选、映射和汇总时，可以先 `filter` 出有效订单，再 `map` 成展示字段，最后 `reduce` 计算总金额。每一步保持单一职责更容易调试。
