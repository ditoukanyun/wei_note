...
- 是否区分自身属性和原型属性。
- 浅拷贝是否满足需求，嵌套对象是否需要深拷贝或不可变更新。
- 是否理解属性描述符中的 writable、enumerable、configurable。
- 是否避免修改内置对象原型。
- 判断对象相等时是否区分引用相等和结构相等。

## 案例

合并配置对象时可以用 `Object.assign` 或展开语法做浅合并；如果配置中有嵌套对象，浅合并会保留内部引用，修改时可能影响原对象。

## 常见误区

- 以为 `Object.assign` 是深拷贝。
- 使用 `for...in` 时把原型链属性也遍历进来。
- 随意 `Object.setPrototypeOf`，影响性能和可读性。
