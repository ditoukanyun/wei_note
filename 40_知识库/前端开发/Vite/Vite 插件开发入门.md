...
Vite 插件开发要同时考虑 dev server 和 build 两条链路。某些逻辑在开发时依赖中间件或 HMR，生产构建时却走 Rollup 打包；只测一边很容易留下环境差异。插件越底层，越要有最小示例项目覆盖常见入口。

性能上要避免对所有模块做重转换。`transform` 钩子应尽量快速判断是否处理目标文件，虚拟模块要有命名空间，配置项要有默认值和类型，方便团队复用和排查。
