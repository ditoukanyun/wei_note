...
- 是否先区分服务器状态、客户端 UI 状态和表单状态。
- 是否先用 React 本地状态解决局部问题。
- 状态更新频率和订阅范围是否匹配工具能力。
- SSR、持久化、调试和团队熟悉度是否纳入选择。
- 是否避免一个项目混用过多状态库。

## 案例

后台管理系统通常可以用 TanStack Query 管接口数据，用 Zustand 管少量客户端状态。只有大型协作应用才更需要 Redux 这类强约束方案。

## 常见误区

- 把接口缓存也放进 Redux。
- 为了流行引入状态库，实际只需要 props 或 context。
- 多个状态源表达同一份数据，导致同步问题。
