...
Suspense 边界本质上是加载体验的切分点。一个边界太大，会让局部等待变成整页闪烁；边界太碎，又会出现多个 fallback 同时跳动。实践中应按用户感知区域划分，例如主内容区、侧边栏、评论区或次要面板。

Suspense 还需要和 Error Boundary 搭配。等待和失败是两类状态：等待时展示 fallback，失败时展示可恢复错误。只处理等待不处理失败，会让网络异常和加载异常变成空白体验。
