...
- atom 是否代表一个清晰、可独立理解的状态单元。
- 派生状态是否优先通过 selector/derived atom 计算。
- 是否避免把服务端状态误放进客户端原子状态。
- atom 数量增加后是否仍能追踪依赖关系。
- 是否有调试工具或命名规范辅助定位状态来源。

## 案例

表单编辑器可以把当前文档、选中节点、侧栏开关和临时输入拆成不同 atom。组件只订阅自己需要的 atom，能减少无关重渲染，也让状态变更路径更清楚。
