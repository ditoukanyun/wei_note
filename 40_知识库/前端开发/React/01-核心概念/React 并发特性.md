---
type: wiki
area: "[[前端开发]]"
tags:
  - React
  - 并发
created: 2026-05-08
---
# React 并发特性

React 并发特性让渲染工作可以被中断、恢复和分优先级处理，从而改善复杂交互下的响应性。

## 相关概念

- [[React 18 新特性]]
- [[Suspense]]
- [[React 性能优化]]

## 工作流程

```mermaid
flowchart TD
  A[触发状态更新] --> B[React 判断优先级]
  B --> C{是否可中断}
  C -->|高优先级| D[优先响应输入]
  C -->|低优先级| E[延后或分片渲染]
  E --> F[提交最终 UI]
```

## 实践检查清单

- 是否用 `startTransition` 标记非紧急更新。
- Suspense 边界是否围绕有意义的加载区域。
- 重渲染问题是否先用 Profiler 定位。
- 是否避免把所有更新都当作 transition，导致反馈变慢。
- 是否理解并发渲染不等于多线程渲染。

## 案例

搜索框输入应保持高优先级响应，搜索结果列表刷新可以放进 transition。这样用户输入不会因为大列表渲染而卡顿。

## 使用边界

React 并发特性解决的是“渲染调度”和“交互响应”问题，不会自动减少业务计算、网络耗时或组件设计错误。若状态放得过高、列表过大或组件副作用混乱，单靠 transition 只能缓解体验，不能替代状态拆分和渲染优化。

落地时应先用 Profiler 找到卡顿来源，再判断哪些更新属于紧急反馈，哪些可以延后。输入框、按钮反馈和焦点状态通常是高优先级；搜索结果、图表重算和大列表刷新更适合低优先级处理。

## 常见误区

- 认为 React 并发特性会自动优化所有组件。
- fallback 设计粗糙，导致局部等待变成整页闪烁。
- 忽略状态边界，导致低优先级更新仍影响整棵树。

## 复盘问题

- 哪些更新必须立即响应用户输入，哪些可以延后。
- 优化前后是否用 Profiler 或用户可感知指标验证。
