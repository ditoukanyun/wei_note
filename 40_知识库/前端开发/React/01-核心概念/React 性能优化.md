...
React 性能优化不能从 `memo`、`useMemo`、`useCallback` 开始。先确认瓶颈来自加载、渲染、数据请求、状态更新还是大列表，再选择对应手段。

## 决策清单

- 首屏慢：先看资源体积、图片、字体、服务端响应和 [[代码分割]]。
- 交互卡：先看长任务、重复渲染、昂贵计算和事件处理。
- 列表慢：优先虚拟列表、分页、增量渲染。
- 状态更新慢：检查状态边界、选择器、组件拆分和 [[TanStack-Query]] 缓存。
- 优化后必须对比 [[Web Vitals 与前端性能监控总览]]，避免只凭主观感觉判断。
