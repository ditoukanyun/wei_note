...
- 每个面试点是否能对应到真实项目问题。
- 是否能解释“为什么这样设计”，而不只是背答案。
- 是否能从构建速度、产物体积、运行时性能三个维度分析优化。
- 是否能画出从 entry 到 bundle 的主流程。
- 是否准备一个自己排查过的 Webpack 问题案例。

## 案例

回答“如何优化 Webpack 构建”时，可以先说定位：用 stats/analyzer 找瓶颈；再说手段：缓存、缩小 Loader 范围、代码分割、Tree Shaking；最后补充验证方式：对比构建时间和产物体积。

## 常见误区

- 只背配置项，不理解构建生命周期。
- 把 Vite、Rollup、Webpack 的机制混为一谈。
- 只讲优化手段，不讲如何量化效果。
