...
同样是 `O(n log n)`，快速排序通常常数小、缓存友好，但遇到极端分区会退化；归并排序稳定、适合链表和外部排序，但需要额外空间；堆排序空间占用低，但局部性通常不如快排。真实工程里还要看数据分布、稳定性要求、内存限制、是否可原地排序以及语言内置排序的行为。

## 检查清单

- 是否要求稳定排序：相等元素的原始顺序是否必须保留。
- 数据规模是否足够小：小数组用插入排序可能更快。
- 数据是否接近有序：插入排序、TimSort 类算法会受益。
- 数据是否为整数且范围有限：可以考虑计数排序、桶排序或基数排序。
- 是否可能出现极端输入：快速排序要考虑随机化、三数取中和尾递归/栈优化。

## 常见误区

- 只记平均时间复杂度，忽略最坏情况和空间复杂度。
- 认为 `Array.prototype.sort` 在所有环境行为完全一致；比较函数和稳定性需要结合运行时版本确认。
- 把计数排序当作通用算法；它依赖有限且可接受的取值范围。
