...
前端并不只在面试中使用算法。虚拟列表需要区间计算，路由和菜单权限会用到树遍历，富文本编辑器涉及栈和区间操作，搜索高亮需要字符串匹配，缓存和去重依赖 [[哈希表]]。算法学习应尽量和真实 UI、数据转换、性能优化场景连接。
...
- 是否能估算时间复杂度和空间复杂度。
- 是否能给出暴力解，再逐步优化。
- 是否覆盖空数组、重复值、边界下标和极端规模。
- 是否复盘题目属于哪类模式，而不只是记答案。
