...
![bf](40_知识库/前端开发/algorithm/img/19.jpg)
...
![rk](40_知识库/前端开发/algorithm/img/20.jpg)
...
```mermaid
flowchart LR
  A[明确模式串和主串] --> B[选择暴力、KMP 或 Sunday]
  B --> C[构造辅助表]
  C --> D[执行匹配]
  D --> E[验证边界样例]
```
...
- 是否处理空模式串、模式串长于主串等边界。
- KMP 的 next 表语义是否统一，避免 off-by-one。
- Sunday 算法是否正确处理主串末尾越界。
- 是否用重复字符样例验证，例如 `aaaaab`。
- 是否根据数据规模选择算法复杂度。

## 案例

在长日志中查找固定错误标记时，若模式串较短且数据量不大，直接使用内置 `indexOf` 即可；算法练习时再手写 KMP 理解前缀表。

## 常见误区

- 只背 KMP 模板，不理解 next 表含义。
- 忽略空字符串和越界访问。
- 在业务代码中重写字符串匹配，放弃内置 API 的可靠性。
