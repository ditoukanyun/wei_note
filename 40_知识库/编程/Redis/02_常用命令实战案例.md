...
- Key 命名是否包含业务前缀和对象 ID。
- 是否为临时数据设置过期时间。
- 是否避免大 Key 和一次性全量读取。
- 批量场景是否使用 MGET/MSET 或 Pipeline。
- 是否为写操作考虑原子性和并发安全。

## 案例

电商活动中，库存用 String 原子递减，购物车用 Hash，排行榜用 Sorted Set，优惠券领取记录用 Set 去重。每类数据结构都对应不同访问模式。

## 常见误区

- 所有值都塞进一个大 JSON，无法局部更新。
- 不设置 TTL，临时活动数据长期占内存。
- 在生产使用 `KEYS *` 排查，阻塞 Redis。
