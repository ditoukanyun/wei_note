---
title: Python工程化实践
description: Python工程化 - Docker容器化、CI/CD、性能优化、部署实践、面试准备（Day91-100）
date: 2026-02-10
tags:
  - python
  - engineering
...
- 是否有单元测试、集成测试和最小冒烟测试。
- 配置是否通过环境变量管理，避免写死密钥。
- 部署是否包含备份、迁移、静态资源和回滚步骤。
- 日志、错误和性能指标是否能支持线上排查。

## 案例延伸

完成 Django 项目后，可以补一个 CI 流程：提交代码后自动运行测试、构建镜像、部署到测试环境，并执行健康检查。这样能把“能运行”提升为“可交付、可回滚、可观测”。
