...
容器运行 Nginx 的重点是让配置可重复，而不是进入容器手工改文件。静态站点、反向代理、HTTPS、gzip、缓存策略和 SPA fallback 都应写进可版本化的配置或镜像构建流程。每次调整后用配置检查和容器日志验证。

前端静态资源要特别注意缓存。HTML 通常不应长缓存，带 hash 的 JS/CSS 可以长缓存；如果策略混乱，用户可能拿到新 HTML 加旧资源，或旧 HTML 加新资源，导致白屏和难复现问题。
