...
- 镜像是否固定版本，而不是默认 `latest`。
- 数据目录和配置文件是否挂载到可管理路径。
- 端口是否只暴露给必要网络。
- 删除容器前是否确认数据卷和备份。
- 常用命令是否整理到项目 README 或 Compose 文件。

## 案例

启动 Redis 时，先创建本地数据目录并挂载，再通过配置文件开启 AOF。这样容器重建后数据仍可恢复，而不是依赖临时容器文件系统。

## 常见误区

- 直接运行容器但不挂载数据，重建后数据丢失。
- 用 `latest` 导致团队环境版本不一致。
- 把本地实验命令原样用于生产环境。
