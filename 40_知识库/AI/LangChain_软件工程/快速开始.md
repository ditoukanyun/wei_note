---
area: [[AI]]
tags:
  - AI
  - LangChain_软件工程
created: 2026-04-30
---
# LangChain 快速开始（知识库原始入口）

概要：LangChain 的安装、核心概念以及快速上手的最小可运行示例。

## 安装与环境
- Python 环境搭建
- pip 安装 LangChain

## 关键概念
- Agents、Chains、Memory、Retrieval、Prompts

## 快速示例
- Hello World 风格的最小示例

## 上手流程

```mermaid
flowchart LR
  A[安装依赖] --> B[配置模型密钥]
  B --> C[创建 Prompt 和 Model]
  C --> D[组合 Chain]
  D --> E[运行并观察输出]
```

## 实践检查清单

- 是否使用独立虚拟环境和锁定依赖版本。
- API Key 是否来自环境变量，不写入笔记或代码。
- 最小示例是否包含输入、模型调用和输出解析。
- 是否记录报错、延迟和成本。
- 跑通后是否连接到 Agents、Retrieval 或 Memory 专题。

## 案例

第一个 LangChain 示例可以只做“输入问题，模型返回回答”。确认链路稳定后，再加入 PromptTemplate 和 Output Parser，避免一开始就混入检索、工具和记忆。

## 常见误区

- 复制复杂示例，遇到错误无法判断是哪一层问题。
- 密钥写在代码或笔记里。
- 没固定依赖版本，教程代码和本地包版本不匹配。

## 复盘问题

- 最小示例是否只包含模型、Prompt、调用和输出解析，便于定位错误。
- 依赖版本、模型配置和环境变量是否可复现。
- 跑通后是否把下一步拆到 Chain、Retrieval、Tools 或 Memory 对应专题。

## 验收标准

快速开始的目标不是覆盖所有能力，而是得到一个可重复运行、可解释失败原因的最小链路。验收时应确认环境变量来自本地配置，依赖版本已记录，输入和输出可复现，错误信息能定位到认证、网络、模型、Prompt 或解析层。跑通后再扩展到 PromptTemplate、Output Parser、Retrieval 或 Tools，每次只增加一个变量。这样后续遇到质量问题时，可以判断是模型能力、上下文质量、链路组合还是工程配置导致的。
