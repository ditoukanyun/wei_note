...
- 中间件顺序是否符合安全、日志、改写、调用的执行顺序。
- 每个中间件是否单一职责，避免同时做鉴权、裁剪和重试。
- 状态修改是否返回框架期望的数据结构。
- 工具调用和模型调用是否分别处理超时、重试和审计。
- 是否为中间件添加单元测试，覆盖正常、异常和空输入。

## 案例

生产 Agent 的中间件顺序可以是权限检查、输入脱敏、消息裁剪、模型调用日志、工具调用审计。权限检查放在最前面，可以避免后续中间件处理不该访问的数据。

## 常见误区

- 中间件里再次调用同一个 Agent，形成递归。
- 日志中记录完整用户输入和工具结果，泄露敏感信息。
- 多个中间件修改同一字段，后执行者覆盖前一个结果。
