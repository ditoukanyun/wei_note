...
- 消息修剪是否保留系统约束、关键事实和最近交互。
- 长期记忆写入是否有用户授权、来源和更新时间。
- 记忆召回是否限制数量，避免噪声覆盖当前任务。

## 案例

客服 Agent 可以用短期记忆保存当前工单上下文，用长期记忆保存客户偏好和历史问题类型。当前对话里的临时折扣方案不应自动写入长期记忆。

## 常见误区

- 把所有历史消息都塞回上下文，导致成本上升和注意力分散。
- 长期记忆没有遗忘机制，旧偏好持续污染新任务。
- 多用户共用同一个会话标识，造成记忆串扰。
