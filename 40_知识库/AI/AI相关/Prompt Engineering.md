...
知识库问答 Prompt 应至少包含：角色、可用上下文、回答格式、引用要求、无法回答时的拒答规则。只写“请根据资料回答问题”通常不够稳定，模型可能忽略引用或编造上下文外信息。

## 工程检查清单

- Prompt 是否有版本号和变更记录。
- 是否保留固定测试集，避免只凭单次输出判断效果。
- 是否区分系统指令、上下文、用户输入和输出格式。
- 是否对用户输入做边界隔离，降低提示词注入风险。
- 是否记录失败案例并反向补充约束。
