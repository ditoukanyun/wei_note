...
XSS 防护的核心是“按输出上下文处理不可信数据”。同一段用户输入进入 HTML 文本、HTML 属性、URL、CSS 或 JavaScript 字符串时，需要不同转义方式。富文本场景不能靠简单替换关键词，应使用白名单清洗，只允许安全标签和属性。

框架默认转义只能覆盖常规文本渲染，无法保护 `dangerouslySetInnerHTML`、模板拼接、第三方脚本和后端直出 HTML。CSP、HttpOnly Cookie 和输入校验是纵深防御，不能替代正确转义。
