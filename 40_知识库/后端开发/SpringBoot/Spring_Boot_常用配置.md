...
配置应该被当作运行时契约管理。新增配置项要说明默认值、适用环境、风险和修改方式；删除或改名配置要考虑兼容期。生产配置尤其要能追踪来源，避免同一个值在 `application.yml`、环境变量、启动参数和配置中心里互相覆盖。

排查配置问题时，可以先确认当前激活的 Profile，再查看最终生效值。很多线上问题不是代码错误，而是超时、连接池、序列化、跨域或日志级别在不同环境不一致。
