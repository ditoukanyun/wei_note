...
- 每个模块是否有启动命令、接口示例和关键知识点总结。
- 学习顺序是否从基础能力逐步过渡到工程治理。
- 每个示例是否明确适用边界和生产注意事项。
- 是否把模块经验拆成原子概念笔记。
- 是否定期复盘哪些模块已经能独立实现。
...
学习认证授权时，可以先跑 Header Token，再跑 Redis Token、JWT Refresh 和 RBAC。这样能逐步比较服务端会话、无状态 Token 和权限模型差异。

## 常见误区

- 按模块跑通代码，但不总结设计取舍。
- 跳过基础集成直接学分布式模式，排查问题时缺少上下文。
- 示例项目完成后没有沉淀到知识图谱。
