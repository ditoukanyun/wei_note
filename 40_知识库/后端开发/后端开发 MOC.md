...
flowchart TD
    ...
    A --> E["稳定性治理"]
    A --> F["交付与观测"]
    B --> B1["REST / OpenAPI / BFF"]
    C --> C1["MySQL / Redis / Elasticsearch"]
    D --> D1["事件 / 一致性 / DDD"]
    E --> E1["超时 / 重试 / 限流 / 降级"]
    F --> F1["Docker / CI-CD / Actuator"]
```

## 使用方法

- 做业务接口前，先看 API 设计与前后端协作。
- 遇到查询慢、搜索弱、缓存故障时，从数据与搜索进入。
- 涉及跨服务状态流转时，从系统设计和最终一致性进入。
- 准备上线或排障时，从 DevOps、可观测性和流量治理进入。
