...
- 本地实验和生产部署是否使用不同密码、端口和数据目录。
- 是否为 Redis 数据卷配置明确路径，避免容器删除后数据丢失。
- 需要持久化时是否开启 AOF、RDB 或混合持久化，并验证恢复。
- 集群或 Sentinel 模式下客户端是否使用对应连接方式。
- 是否限制 Redis 暴露端口，避免直接暴露到公网。

## 案例

本地开发只需要单节点 Redis 时，用 Compose 固定容器名、端口、密码和数据卷即可。若要验证主从切换，再单独使用 Sentinel 或 Cluster 配置，不要把实验集群配置直接搬到生产。

## 常见误区

- 容器能启动就认为 Redis 高可用，实际没有主从、哨兵或备份。
- 端口直接映射到外网且没有密码。
- 使用临时容器存关键数据，重建后状态丢失。
